...
- **Independent backends do not inherit Kiro readiness.** Codex/Claude model
  discovery returns their advertised catalog, and Kiro usage polling returns
  `{"usage": null}` without resolving or spawning Kiro. Destructive reruns and
  the compatibility API apply the Kiro guard only to Kiro-family selections.
  Explicit Kiro spawn sites pass `kiro_spawn=True` to retain their fail-closed
  check even if the configured backend changes during the request.
